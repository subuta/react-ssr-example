...
import { dev } from 'lib/common/utils/env'

const PAGES_INDEX_TEMPLATE_FILE = path.resolve(LIB_DIR, './common/utils/page.js')
const PAGES_INDEX_FILE = path.resolve(PAGES_DIR, './index.js')

// Run /pages watcher if dev and `DISABLE_WATCH_PAGES` not set.
if (dev && !process.env.DISABLE_WATCH_PAGES) {
...
export const syncPages = () => {
  ...
  const defs = _.map(pages, (page) => (
    `const ${getPageName(page)} = loadable(async () => wrapLoadable(await import('.${page}')), { render: renderLoadable })`
  )).join('\n')
